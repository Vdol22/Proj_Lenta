def forecast(sales: dict, item_info: dict, store_info: dict) -> list:
    ...
    # Удалить отрицательные значения в столбцах 'pr_sales_in_units', 'pr_promo_sales_in_units', 'pr_sales_in_rub' и 'pr_promo_sales_in_rub'
    df_num = df_num.loc[(df_num['pr_sales_in_units'] >= 0) & (df_num['pr_promo_sales_in_units'] >= 0) & (df_num['pr_sales_in_rub'] >= 0) & (df_num['pr_promo_sales_in_rub'] >= 0)]
    ...
    # У нас представлено очень маленькое количество товаров 7 и 8 группы. Такие группы следует исключить из дальнейшего исследования.
    ...
    meta_model = pickle.loads('meta_model.pkl')
    ...
    meta_feat_array_test, features_df_test = predict_model_1st_level(test, all_models)
